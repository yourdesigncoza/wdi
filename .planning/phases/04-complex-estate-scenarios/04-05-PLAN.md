...
<task type="auto">
  ...
  <action>
    **WillWizard.tsx** — Extend to handle complex sections:
    ...
    3. Update `renderSection()` switch/conditional to include cases for complex sections:
    ...
    4. Add scenario detection trigger as an interstitial inside WillWizard:
       - Add a local state `scenariosDetected: boolean` (default false)
       - When user navigates past residue AND `scenariosDetected === false`, render ScenarioDetector component instead of the next section
       - ScenarioDetector's `onContinue` callback: set `scenariosDetected = true`, then navigate to first complex section (from scenarios array) or 'review' if scenarios empty
       - Read scenarios from store (`useWillStore((s) => s.scenarios)`) to determine which complex sections to render
       - Once scenariosDetected is true, normal navigation resumes to complex sections then review
  ...
  <verify>
    ...
    - WillWizard renderSection() switch/conditional includes cases for 'trust', 'usufruct', 'business', 'joint' that return the corresponding component with willId prop
    - Scenario detection logic triggers after residue completion: check for `scenariosDetected === false` condition that shows ScenarioDetector interstitial
    - Complex sections are conditionally rendered based on scenarios array from store (not always shown)
    - ensureWillExists covers new AI sections (trust, usufruct, business in AI_SECTIONS set)
    - SectionReview shows trust, usufruct, business, joint data when present (conditionally, based on data existence)
